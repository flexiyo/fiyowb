...
export default async function musicSeo(request, env) {
  ...
  try {
    ...
    const html = renderSeoPage(template, {
      title: data.title,
      description: `Listen and download '${data.title}' by ${data.artists
        .split("•")[0]
        .trim()} in high quality - only on Flexiyo.`,
      ...
      content_block: `
          ...
          ${
            image
              ? `<figure><img src="${image}" alt="${data.title}" /></figure>`
              : ""
          }
        `,
    });
